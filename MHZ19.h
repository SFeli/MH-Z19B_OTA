...
class MHZ19
{
  public:
	...
	/* reads range using command 125 / 0x7D */
	int getABC();	
  ...
  /* alias for command types */
	typedef enum COMMAND_TYPE
	{
		...
		ABC = 1,		// 1 Set ABC Mode ON/OFF
		GETABC = 2,		// 2 Get ABC - Status 0x79
		RAWCO2 = 3,		// 3 Get Raw CO2
		CO2UNLIM = 4,           // 4 Get Temp for unsigned, CO2 Unlimited
		CO2LIM = 5,	     	// 5 Get Temp for signed, CO2 limited
		ZEROCAL = 6,	  	// 6 Zero Calibration
		SPANCAL = 7,	  	// 7 Span Calibration
		RANGE = 8,		// 8 Set Range    0x99
		GETRANGE = 9,	        // 9 Get Range    0x9B
